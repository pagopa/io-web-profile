...
import { SetWalletInstanceStatusDataEnum } from './generated/wallet/SetWalletInstanceStatusData';
...
const withCustomToken: WithDefaultsT<'bearerAuth'> = (wrappedOperation: any) => (params: any) => {
  const token = IS_MOCK_USER_ENABLED
    ? global.window.localStorage.getItem('customToken')
...
export const WebWalletApi = {
  ...
  getIsFiscalCodeWhitelisted: async () => {
    const result = await webWalletApiClient.isFiscalCodeWhitelisted({});
    return extractResponse(result);
  },
};
